...
def random_element(my_2d_list):
    matching_coordinates = [(i, j) for i, row in enumerate(my_2d_list) for j, val in enumerate(row) if val == ' ']
    
    if matching_coordinates:
        random_coordinate = random.choice(matching_coordinates)
        return[random_coordinate[0], random_coordinate[1]]
      ...
      while win != True and any(' ' in row for row in placed):
          ...
          if cur == 'X':
            ...
            if placed[row-1][col-1] == " ":
              ...
              re = random_element(placed)
              row = re[0] + 1
              col = re[1] + 1
              placed[row - 1][col - 1] = 'O'
